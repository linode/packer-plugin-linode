...
runtime in order to build packer with the Linode plugin.
...
With Go installed, you can already `go get` the Linode plugin and `make dev` in
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-linode f-my-feature
```
...
3. One of Linode plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Linode plugin
project. After following the steps in "Setting up Go to work on the Linode plugin":
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-linode.git`
...
Please only apply the minimal vendor changes to get your PR to work. The Linode plugin
...
If the Linode Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
